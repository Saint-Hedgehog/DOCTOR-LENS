  ...
  class SelectConstructor {
    constructor(props, data = null) {
      ...
      this.selectClasses = {
        ...
        classSelectDisabled: 'select-disabled', // Запрешен
    ...
    selectInit(originalSelect, index) {
      ...
      originalSelect.addEventListener('change', function (evt) {
        thisElement.selectChange(evt);
      });
    }
    ...
    selectBuild(originalSelect) {
      ...
      selectItem.classList.add(originalSelect.getAttribute('class') ? `select--${originalSelect.getAttribute('class')}` : '');
    ...
    selectsActions(evt) {
      ...
      if (targetElement.closest(this.getSelectClass(this.selectClasses.classSelect)) || targetElement.closest(this.getSelectClass(this.selectClasses.classSelectTag))) {
        ...
        } else if (targetType === 'keydown' && evt.code === 'Escape') {
          this.selectsСlose();
        }
      } else {
        this.selectsСlose();
    ...
    getSelectTitleValue(selectItem, originalSelect) {
      ...
      if (originalSelect.hasAttribute('data-search')) {
        ...
        <span class="${this.selectClasses.classSelectValue}"><input autocomplete="off" type="text" placeholder="${selectTitleValue}" data-placeholder="${selectTitleValue}" class="${this.selectClasses.classSelectInput}"></span></div>`;
      } else {
        // Если выбран элемент со своим классом
        const customClass = this.getSelectedOptionsData(originalSelect).elements.length && this.getSelectedOptionsData(originalSelect).elements[0].dataset.class ? ` ${this.getSelectedOptionsData(originalSelect).elements[0].dataset.class}` : '';
        // Выводим текстовое значение
        return `<button type="button" class="${this.selectClasses.classSelectTitle}"><span class="${this.selectClasses.classSelectValue}"><span class="${this.selectClasses.classSelectContent}${customClass}">${selectTitleValue}</span></span><span class="select__text-error"></span></button>`;
      }
    }
    ...
    getSelectedOptionsData(originalSelect) { // type второй параметр
    ...
    optionAction(selectItem, originalSelect, optionItem) {
      if (originalSelect.multiple) { // Если мультивыбор
        ...
        // Выделяем классом элемент
        const selectClasOptions = document.querySelectorAll('.select__option');
        selectClasOptions.forEach((element) => {
          if (element.classList.contains('select-selected')) {
            element.classList.remove('select-selected');
            element.removeAttribute('tabindex');
          }
        });
        optionItem.classList.add('select-selected');
    ...
    selectChange(evt) {
      const originalSelect = evt.target;
      this.selectBuild(originalSelect);
      this.setSelectChange(originalSelect);
    }
    ...
    searchActions(selectItem) {
      ...
      selectInput.addEventListener('input', function () {
        ...
        if (selectOptions.hidden === true) {
          thisElementElement.selectAction(selectItem);
        }
      });
    }
    // Коллбэк функция
